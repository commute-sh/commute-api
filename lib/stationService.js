...
exports.findByIds = function(contractName, stationIds) {

    const cache = Cache.create();

    console.log(`[INFO] Searching stations ids: ${stationIds}`);

    if (stationIds.length === 0) {
        return [];
    }

    const keys = stationIds.map(stationId => `${contractName}_${stationId}`);
