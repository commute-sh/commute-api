...
exports.nearby = function(contractName, lat, lng, distance) {
    ...
    return cache.georadiusAsync(`${contractName}_stations`, lng, lat, 100, 'km', 'WITHDIST', 'ASC')
        .then((entries) => {
            ...
            const keys = entries.map(([stationNumber, _]) => `${contractName}_${stationNumber}`);

            console.log("*** entries.map:", moment.duration(moment().diff(start)).milliseconds(), "ms");
